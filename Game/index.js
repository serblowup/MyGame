...
const WIDTH = width();
const HEIGHT = height();
const FLOOR_HEIGHT = HEIGHT / 4;
const JUMP_FORCE = 900;
const SPEED = 500;
const BACKGROUND_COLOR = Color.fromHex("#6495ED");
const GRASS_COLOR = Color.fromHex("#b3bfae");
const FLOOR_COLOR = Color.fromHex("#c9d2c6");
...
scene("MainMenu", function () {
    loadSprite("BackgroundMainMenu", "BackgroundMenu.jpg");
...
scene("game", function (InitialScore, playerState, blocksState) {
    ...
    const createPlayer = function (state) {
        const player = add([
            ...
            scale(1),
            pos(80, HEIGHT - FLOOR_HEIGHT - 40),
            area(),
            body(),
            { currentFrame: 0, isPaused: false }]);
        if (state) {
